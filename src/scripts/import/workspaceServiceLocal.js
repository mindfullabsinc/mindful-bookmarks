 ...
 */
function mapToBookmarkGroups(groups) {
    const now = Date.now();
    return groups.map((group) => {
        const bookmarks = group.items.map((item) => ({
            id: item.id,
 ...
 */
export function createWorkspaceServiceLocal(userId) {
    return {
        /**
         ...
         */
        async createWorkspaceForPurpose(purpose) {
         ...
         */
        async saveGroupsToWorkspace(workspaceId, groups) {
            ...
            const fullStorageKey = wsKey(workspaceId, groupsKey);
            // 1) Write all groups for this workspace
            await LocalAdapter.writeAllGroups(workspaceId, fullStorageKey, bookmarkGroups);
            // 2) Update first-paint + session mirrors for fast reopen / WS switching
            await LocalAdapter.persistCachesIfNonEmpty(workspaceId, bookmarkGroups);
        },
        /**
         ...
         */
        async appendGroupsToWorkspace(workspaceId, groups) {
            ...
            const existing = obj?.[fullStorageKey] ?? [];
            const merged = [...existing, ...newGroups];
            await LocalAdapter.writeAllGroups(workspaceId, fullStorageKey, merged);
            await LocalAdapter.persistCachesIfNonEmpty(workspaceId, merged);
        }
    };
}
