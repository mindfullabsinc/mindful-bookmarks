...
/* Scripts and hooks */
import { getActiveWorkspaceId, listLocalWorkspaces, createLocalWorkspace, renameWorkspace, archiveWorkspace, } from '@/scripts/workspaces/registry';
        ...
        _jsxs("div", { ref: panelRef, id: "ws-panel", role: "dialog", "aria-modal": "false", "aria-label": "Workspace switcher", className: `\n          fixed left-0 top-0 z-50 h-full w-[280px] max-w-[85vw]\n          rounded-r-2xl border border-l-0\n          bg-white text-neutral-900 border-neutral-200\n          shadow-2xl backdrop-blur\n          transition-transform duration-200\n          dark:bg-neutral-900 dark:text-neutral-100 dark:border-neutral-700\n          ${panelOpen ? 'translate-x-0' : '-translate-x-full'}\n        `, children: [
            ...
            _jsxs("div", { className: "max-h-[calc(100vh-9rem)] overflow-auto p-3", children: [
                ...
                workspaces.map((w) => {
                    ...
                    const rowClass = 'group flex items-center justify-between gap-2 px-3 py-2 rounded-xl ' + (isActive ? 'bg-blue-600 text-white' : 'bg-black/5 hover:bg-black/10 dark:bg-white/5 dark:hover:bg-white/10');
                    const spanClass = 'flex-1 text-sm outline-none cursor-text min-w-0';
                    return (_jsxs("div", { className: rowClass, children: [
                        isEditing
                            ? _jsx("span", {
                                ref: editSpanRef,
                            ...
                            : _jsx("button", { onClick: () => handleSwitch(w.id), className: "flex-1 text-left text-sm truncate cursor-pointer", "aria-current": isActive ? 'true' : undefined, "aria-label": w.name, title: w.name, children: _jsxs("div", { className: "flex items-center justify-between gap-3", children: [
                        ...
                        _jsx("div", { className: "flex gap-0.5 shrink-0", children: isEditing
                            ...
                            : _jsxs(_Fragment, { children: [
                                _jsx("button", { onClick: () => startEdit(w.id), className: "text-[11px] px-1 py-1 rounded-md hover:bg-black/10 dark:hover:bg-white/10 cursor-pointer", "aria-label": `Rename ${w.name}`, title: "Rename", children: _jsx("i", { className: "fa-solid fa-pen" }) }),
            ...
            _jsxs("footer", { className: "border-t border-neutral-200 dark:border-white/10 px-3 py-3 rounded-b-2xl", children: [
                _jsx("button", { onClick: handleCreate, className: "w-full text-left text-sm px-3 py-2 rounded-xl cursor-pointer\n                       bg-black/5 hover:bg-black/10\n                       dark:bg-white/5 dark:hover:bg-white/10", children: _jsxs("span", { className: "inline-flex items-center gap-2", children: [_jsx("i", { className: "fa-solid fa-plus" }), _jsx("span", { children: "New Local Workspace" })] }) }),
